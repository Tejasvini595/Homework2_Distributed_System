// log_analytics_seq.cpp
// Q7 — Large-Scale Server Log Analytics (SEQUENTIAL implementation)
//
// Input:
//   First line: N K S
...
//
// Usage:
//   ./log_analytics_seq < input.txt > output.txt
//   ./log_analytics_seq input.txt output.txt
//
// Compilation:
...
int main(int argc, char** argv) {
    ...
    for (long long i = 0; i < N; i++) {
        ...
        totalRequests++;
        if (status_code < 400) successful++; else failed++;

        sumResponseTime += response_time;
        if (response_time < minResponseTime) minResponseTime = response_time;
        if (response_time > maxResponseTime) maxResponseTime = response_time;
