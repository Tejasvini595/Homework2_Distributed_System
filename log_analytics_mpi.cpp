...
// Busiest interval id = timestamp / 60 (integer division).
// Top-K results are sorted by decreasing count, then increasing ID.
//
// Parallelization strategy:
//   1. Data distribution is done as PARALLEL FILE I/O, not by having one
//      rank read everything and ship it out: every rank independently
//      opens the same input file (it sits on a shared filesystem) and
//      claims a contiguous BYTE range of size (file_size / P). Each rank
//      then snaps its start/end offsets forward to the next line boundary
//      (skip_to_line_start), so every log line is parsed by exactly one
//      rank and no MPI communication is needed to hand out the raw data
//      at all -- this is the classic "file split" pattern used by
//      big-data log processors (e.g. Hadoop's TextInputFormat) and it
//      keeps read bandwidth scaling with P instead of bottlenecking on a
//      single rank's disk/parsing throughput.
//      An earlier version of this program had rank 0 read + parse the
//      whole file and Scatterv the columns out; measured locally, that
//      made rank 0's serial read dominate total time and produced no
//      speedup at all (see README's I/O bottleneck note) -- hence this
//      design.
//   2. Each rank parses its own line range directly off disk and
//      accumulates local partial statistics in one pass: running sums/
//      min/max for response time, byte total, per-status-class counts,
//      and three local hash maps (server -> {count,sum_rt}, endpoint ->
//      {count,total_bytes}, interval -> count).
//   3. Reduction: the small number of scalars use MPI_Reduce (SUM / MIN /
//      MAX). The three hash maps are combined by gathering each rank's
//      distinct (key, value...) rows to rank 0 with MPI_Gather +
//      MPI_Gatherv (sizes are proportional to the number of distinct
//      servers/endpoints/intervals, not to N, so this stays cheap even
//      for huge logs) and merging them into global maps on rank 0.
//   4. Rank 0 sorts for Top-K and busiest interval, and writes the output.
//
// Usage:
...
// Return the offset of the start of the next full line at or after byte
// offset `b` (i.e. skip past whatever partial line straddles `b`).
...
static long long skip_to_line_start(ifstream &fin, long long b, long long data_start, long long file_size) {
    if (b <= data_start) return data_start;
    if (b >= file_size) return file_size;
...
int main(int argc, char **argv) {
    ...
    // --------------------------------------------------------------
    // Every rank opens the file itself and figures out its own byte
    // range -- no communication needed to distribute the raw records.
    // --------------------------------------------------------------
    ifstream fin(argv[1]);
    if (!fin) {
    ...
    while (iss >> timestamp >> server_id >> endpoint_id >> user_id_unused
        ...
        localN++;
        if (status_code < 400) localSuccessful++; else localFailed++;

        localSumResponseTime += response_time;
        if (response_time < localMinResponseTime) localMinResponseTime = response_time;
        if (response_time > localMaxResponseTime) localMaxResponseTime = response_time;
    ...
    // --------------------------------------------------------------
    // Gather + merge the three hash maps (server, endpoint, interval).
    // Each rank sends its distinct keys and per-key aggregates; sizes
    // scale with the number of distinct keys, not with N.
    // --------------------------------------------------------------
    auto gatherServerMap = [&](unordered_map<int, ServerStats> &local) -> unordered_map<int, ServerStats> {
        ...
        vector<double> sums; sums.reserve(localCount);
        for (auto &kv : local) {
        ...
        int total = 0;
        if (rank == 0) {
            for (int p = 0; p < P; p++) { recvDispls[p] = total; total += recvCounts[p]; }
        }
        ...
        MPI_Gatherv(sums.data(), localCount, MPI_DOUBLE,
                    allSums.data(), recvCounts.data(), recvDispls.data(), MPI_DOUBLE, 0, MPI_COMM_WORLD);
    ...
    if (rank == 0) {
        ...
        cerr << "Parallel read+compute time (max over ranks reported by rank 0's own timer): "
             << (t_io_compute_end - t_io_compute_start) << " sec\n";
        cerr << "Reduction/merge time:      " << (t_reduce_end - t_reduce_start) << " sec\n";
        cerr << "Total time:                " << (t_total_end - t_io_compute_start) << " sec\n";
    }
